...
export default new PageModule ({
    ...
    buildPage: async function({args={}, data={}}={}) {
        ...
        await componentPage.addComponent(ComponentPage.componentTypes.SETTINGS.USER_MAIL, {allowEdit: true, size: "full", order: 1}, {user: user, targetUser: "current"});
        await componentPage.addComponent(ComponentPage.componentTypes.SETTINGS.USER_MAIL_PASSWORD, {allowEdit: true, size: "full", order: 1}, {user: user, targetUser: "current"});
        if(pageArgs.acl.docker.system.mail){
            await componentPage.addComponent(ComponentPage.componentTypes.SETTINGS.USER_MAIL_DEV, {allowEdit: true, size: "full", order: 1}, {user: user, targetUser: "current"});
        }
