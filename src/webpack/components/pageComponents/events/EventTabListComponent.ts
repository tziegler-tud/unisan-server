...
import "../../scss/events/EventTabListComponent.scss"


interface TabEventDetail {
    index: number;
}
...
export default class EventTabListComponent extends Component {
    /**
     *
    ...
    constructor({
        ...
        this.templateUrl = "/webpack/components/templates/events/eventTabList.hbs";
    }
    ...
    private upcomingTabContainer: HTMLElement | null;
    private pastTabContainer: HTMLElement | null;
    ...
    async postRender(): Promise<void> {
        ...
        this.upcomingTabContainer = document.querySelector('.eventTabComponent-container__upcoming');
        this.pastTabContainer = document.querySelector('.eventTabComponent-container__past');
        ...
        document.addEventListener('MDCTabBar:activated', (event) => {
            // @ts-ignore
            const detail: TabEventDetail = event.detail;
            switch (detail.index) {
                case 0:
                    this.activateUpcoming()
                    break;
                case 1:
                    this.activatePast()
                    break;
                default:
                    console.error("Invalid tab index received: ", detail.index)
            }
        })
    ...
    activateUpcoming(): boolean {
        this.upcomingTabContainer.classList.add("eventTabComponent-container--active");
        this.pastTabContainer.classList.remove("eventTabComponent-container--active");
        return true
    }

    activatePast(): boolean {
        this.upcomingTabContainer.classList.remove("eventTabComponent-container--active");
        this.pastTabContainer.classList.add("eventTabComponent-container--active");
        return true
