...
export default class CreateNewsComponent extends Component {
    ...
    async postRender(){
        ...
        this.titleField = new EditableInputField(titleFieldContainer, this.data.news.content.delta, this.data.news.content.html, "text", callback, {readOnly: false});
        let contentFieldContainer = self.container.querySelector(".news-content--input");
        this.contentField = new EditableTextField(contentFieldContainer, this.data.news.content.delta, this.data.news.content.html, callback, {readOnly: false});
    }
